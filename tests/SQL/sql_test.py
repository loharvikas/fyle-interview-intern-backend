...
def test_get_assignments_in_various_states(app_ctx):
    """Test to get assignments in various states"""

    # Define the expected result before any changes
    expected_result = [('DRAFT', 3), ('GRADED', 2), ('SUBMITTED', 2)]
    ...
    with app_ctx.app_context():
        ...
        expected_result = [('DRAFT', 3), ('GRADED', 3), ('SUBMITTED', 1)]

        # Find an assignment in the 'SUBMITTED' state, change its state to 'GRADED' and grade to 'C'
        submitted_assignment: Assignment = Assignment.filter(Assignment.state == AssignmentStateEnum.SUBMITTED.value).first()
        submitted_assignment.state = AssignmentStateEnum.GRADED.value
...
def test_get_grade_A_assignments_for_teacher_with_max_grading(app_ctx):
